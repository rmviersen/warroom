-- Rename ``war`` to component WPR columns; extend batting with rollup placeholders.

ALTER TABLE public.player_batting_seasons
  RENAME COLUMN war TO bwpr;

ALTER TABLE public.player_batting_seasons
  ADD COLUMN fwpr NUMERIC(6, 1),
  ADD COLUMN brwpr NUMERIC(6, 1),
  ADD COLUMN wpr NUMERIC(6, 1);

COMMENT ON COLUMN public.player_batting_seasons.bwpr IS
  'Batting-only wins above replacement (offensive batting WPR) for this player-season. Populated when the batting season-metrics pipeline writes it; was formerly ``war``.';
COMMENT ON COLUMN public.player_batting_seasons.fwpr IS
  'Fielding WPR contribution. Null until the fielding WPR pipeline is built.';
COMMENT ON COLUMN public.player_batting_seasons.brwpr IS
  'Baserunning WPR contribution. Null until the baserunning WPR pipeline is built.';
COMMENT ON COLUMN public.player_batting_seasons.wpr IS
  'Total positional-player WPR (batting + fielding + baserunning once computed). Null until all relevant pipeline components exist and total is populated.';

ALTER TABLE public.player_pitching_seasons
  RENAME COLUMN war TO pwpr;

COMMENT ON COLUMN public.player_pitching_seasons.pwpr IS
  'Pitching wins above replacement (pitching WPR) for this player-season. Populated when the pitching metrics pipeline computes it; was formerly ``war``.';
...
BEGIN
  ...
  LOOP
    ...
    DO UPDATE SET
      ...
      bwpr = COALESCE(EXCLUDED.bwpr, player_batting_seasons.bwpr),
      fwpr = COALESCE(EXCLUDED.fwpr, player_batting_seasons.fwpr),
      brwpr = COALESCE(EXCLUDED.brwpr, player_batting_seasons.brwpr),
      wpr = COALESCE(EXCLUDED.wpr, player_batting_seasons.wpr),
